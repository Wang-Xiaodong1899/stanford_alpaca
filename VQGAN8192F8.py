...
class Args(BasicArgs):
    ...
    pretrained_model = os.path.join(BasicArgs.root_dir, 'vqg/VQGan8192F8.pth')
    ...
    coder_config = {'double_z': False, 'z_channels': 256, 'resolution': img_size, 'in_channels': 3, 'out_ch': 3, 'ch': 128,
                    'ch_mult': [1, 1, 2, 4], 'num_res_blocks': 2, 'attn_resolutions': [32], 'dropout': 0.0}

    quantize_config = {'n_embed': 8192, 'embedding_dim': 256, 'num_hiddens': 256, 'beta': 0.25, 'remap': None,
                       'sane_index_shape': False, 'legacy': False, 'kl_weight': 1.0e-08}
...
class Net(nn.Module):
    def __init__(self, args, mode='train'):
        ...
        self.quantize = GumbelQuantize(**args.quantize_config)
        self.quant_conv = torch.nn.Conv2d(args.coder_config["z_channels"], args.quantize_config["embedding_dim"], 1)
        self.post_quant_conv = torch.nn.Conv2d(args.quantize_config["embedding_dim"], args.coder_config["z_channels"], 1)
    ...
    def decode(self, img_seq, height=None):
        b, n = img_seq.shape
        img_seq = rearrange(img_seq, 'b (h w) -> b h w', h=height if height else int(math.sqrt(n)))
        z = self.quantize.get_codebook_entry(img_seq)
